...
public class ArgumentParser {
    ...
    /** Will determine if a parsed Argument HashMap contains the required flags. If it does not
     * contain a single required flag this will return false after sending a message to the player.
     * @param commandSender - Who sent the command
     * @param requiredArguments - Hashset of required flags
     * @param parsedArguments - HashMap of parsedArguments
     * @param usage - String[] containing usage information for the player
     * @return 
     */
    public static boolean validateCommandInput(CommandSender commandSender, HashSet<String> requiredArguments, HashMap<String, String> parsedArguments, String[] usage){
        for (String argument : requiredArguments){
            if (!parsedArguments.containsKey(argument.toLowerCase())) {
                commandSender.sendMessage(ChatColor.RED + " > Error: No -" + argument+ " flag found! Usage: ");
